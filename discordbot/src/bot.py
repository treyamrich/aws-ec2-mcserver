...
async def start(ctx):
	...
	if not instance.isNew:
		...
		_logger.debug(f"Server is already running at **{config.SERVER_ADDRESS}**")
		return

	desc = ":desktop: Server is booting up. Pls wait __5 min__ :pray:"
	_logger.info("Server boot initiated")
...
# 		_logger.info(f"Executed list_players. Online players: {online_players}")
...
@bot.event
async def on_ready():
    _logger.info(f'{bot.user.name} has connected to Discord!')
